		...
		try:
			...
			if is_private:
				self.S3_CLIENT.upload_file(
					file_path, self.BUCKET, key, ExtraArgs={"ContentType": content_type, "Metadata": metadata}
				)
			else:
				self.S3_CLIENT.upload_file(
					file_path, self.BUCKET, key, ExtraArgs={"ContentType": content_type, "Metadata": metadata}
...
def file_upload_to_s3(doc, method):
	...
	if parent_doctype not in ignore_s3_upload_for_doctype:
		...
		key = s3_upload.upload_files_to_s3_with_key(
			file_path, doc.file_name, doc.is_private, parent_doctype, parent_name, doc.name
		)
